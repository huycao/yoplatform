        ...
        <div class="box-body">
            ...
            <div class="">        
                <div role="alert" class="alert alert-success fade in">
                  ...
                  <p class="text-success">{{Session::get('mess')}}</p>
                </div>
            </div>
            @endif
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control input-sm" {{{ (isset($id) && $id>0) ? "disabled" : "" }}} id="username" value="{{{ $item->username or Input::get('username') }}}" name="username" placeholder="Username">
                    ...
                    <div class="row">
                        <div class="col-xs-3">
                            <div class="form-group">
                                <label>{{trans('backend::publisher/text.status')}}</label>
                                <select class="form-control input-sm" id="status" name="status">
                                    <option value="1" <?php if( isset($item->activated) &&  $item->activated == 1 ){ echo "selected='selected'"; }?> >Active</option>
                                    <option value="0" <?php if( isset($item->activated) &&  $item->activated == 0 ){ echo "selected='selected'"; }?> >Off</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-xs-3">
                            <div class="form-group">
                                <label>{{trans('backend::publisher/text.role')}}</label>
                                <select class="form-control input-sm" id="role" name="role">
                                    <option value="2" <?php if( isset($item->groups_id) &&  $item->groups_id == 2 ){ echo "selected='selected'"; }?> >Manager Publisher</option>
                                    <option value="4" <?php if( isset($item->groups_id) &&  $item->groups_id == 4 ){ echo "selected='selected'"; }?> >Publisher</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-sm-6">
                    ...
                    <div class="form-group">
                        ...
                        <select name="country" class="form-control input-sm" id="country">
                            ...
                            <option {{{ (isset($item->country_id) && $value->id==$item->country_id) ? "selected" : "" }}} value="{{$value->id}}">{{$value->country_name}}</option>
                            @endforeach
                        </select>
