...
class AgencyAdvertiserManagerController extends AdvertiserManagerController 
{

	public function __construct(AgencyAdvertiserManagerModel $model) {
		...
		if( $validate->passes() ){
			...
			if( $id == 0 ){ // INSERT
				...
				if( $item = $this->model->create($updateData) ){
                    ...
                    Session::flash('flash-message', 'Create Agency Success!');
					return TRUE;
				}
			}else{ // UPDATE
				...
				if( $item ){
					if( $this->model->where("id",$id)->update($updateData) ){
                    	Session::flash('flash-message', 'Update Agency Success!');
						return TRUE;
					}
				}
			}
