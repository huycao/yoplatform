...
class FlightBaseModel extends Eloquent {
  ...
  public static function getDailyInventories($campaignIds, $flightIds, $adIds) {
    $inventories = [];
    $typeapp = 'site';
    $date = date('Y-m-d');
    $rawDatas = TrackingSummaryBaseModel::getDatasPerDate($date, $campaignIds, $adIds, $flightIds, $typeapp);
    foreach ($rawDatas as $datas) {
      $retval = null;
      if (isset($datas['summary']) && count($datas['summary']) > 0) {
        $sumImpression = $sumImpressionOver = $sumUniqueImpression = $sumUniqueImpressionOver = $sumClick = $sumClickOver = $sumUniqueClick = $sumUniqueClickOver = 0;
        foreach ($datas['summary'] as $data) {
      ...
      //var_dump( $datas);      
      $strIndex = $datas['campaign']['id']."_".$datas['flight']['id'];
      $inventories[$strIndex] = $retval;
    }
    return $inventories;
  }
